    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                user = new ApplicationUser { UserName = Input.Email, Email = Input.Email, FirstName = Input.FirstName, LastName = Input.LastName,MobileNo= Input.MobileNo, RoleName = Input.RoleName, Created = DateTime.Now, CreatedBy = User.Identity.Name, LastUpdated = DateTime.Now, LastUpdatedBy = User.Identity.Name };
                ...
                if (result.Succeeded)
                {
                    ...
                    var confirmLink = $"Confirm here: {HtmlEncoder.Default.Encode(callbackUrl)}\n\n Click the link above to confirm your email.";
                    var message =
                         $"Confirm Your Email\n\n" +
                         $"Dear {user.FirstName},\n\n" +
                         $"We noticed that you've requested to create your account with us.\n\n" +
                         $"If you didn't request an account create, please disregard this email.\n\n" +
                         $"For your security, please do not share this email with anyone. The link will expire in 20 minutes, so be sure to confirm your email as soon as possible.\n\n" +
                         $"To confirm your email, simply click the link below:\n" +
                         $"{confirmLink}\n\n" +
                    ...
                    await emailHelper.SendEmailAsync(Input.Email, "Confirm your email", message);
